...
class CallICFGNode(ICFGNode):
    ...
    def getCaller(self) -> "FunObjVar": ...
    """Get the caller function object. To get the ICFG entry node, call icfg.getFunEntryICFGNode() on the result."""
...
class IntraCFGEdge(ICFGEdge):
    ...
    def getCondition(self) -> Optional["SVFVar"]: ...
    """Get the condition variable, or None for unconditional edges"""
...
class GepStmt(AssignStmt):
    ...
    def getStructFieldOffset(self, idx_var: "ValVar", struct_type: "SVFStructType") -> int: ...
...
class MultiOpndStmt(SVFStmt):
    ...
    def getOpVarId(self, ID: int) -> int: ...
...
class SVFIR(SVFLLVMValue):
    ...
    def getSVFVarNum(self) -> int: ...
    """Get the number of SVFVars (PAG nodes)"""
...
class SVFG(VFG):
    ...
    def hasDefSVFGNode(self, val: ValVar) -> bool: ...
    def getDefSVFGNode(self, val: ValVar) -> List[VFGNode]: ...
    ...
    def getSVFGNode(self, id: int) -> VFGNode: ...
    """Get an SVFG node by ID"""
    def hasSVFGNode(self, id: int) -> bool: ...
    """Check if an SVFG node exists"""
    def getDefSiteOfValVar(self, var: ValVar) -> "VFGNode": ...
    """Get the SVFG definition site of a value variable"""
    def getDefSiteOfObjVar(self, obj: ObjVar, node: "VFGNode") -> List["VFGNode"]: ...
    """Get the SVFG definition sites of an object variable reaching this SVFG node"""
    def getUseSitesOfValVar(self, var: ValVar) -> List["VFGNode"]: ...
    """Get the SVFG use sites of a value variable"""
    def getUseSitesOfObjVar(self, obj: ObjVar, node: "VFGNode") -> List["VFGNode"]: ...
    """Get the SVFG use sites of an object variable starting from this SVFG def-site node"""
    def isFunEntrySVFGNode(self, node: VFGNode) -> Optional[FunObjVar]: ...
    """Check if the SVFG node is a function entry node, returns the function or nullptr"""
    def isCallSiteRetSVFGNode(self, node: VFGNode) -> Optional[CallICFGNode]: ...
    """Check if the SVFG node is a call site return node, returns the call site or nullptr"""
...
class IntervalValue:
    ...
    """True only when this interval is the definite/concrete truth value [1, 1];
    False for [0, 0], ambiguous ranges (e.g. [0, 1]), top, and bottom."""
...
class AbstractState:
    ...
    """Not a real SVF C++ AbstractState/AbstractInterpretation API: this is a pysvf-only
    static helper that reimplements AbstractInterpretation::isCmpBranchFeasible's logic
    in the Python bindings, since that method is private in the C++ library."""
    @staticmethod
    def isSwitchBranchFeasible(svfir: 'SVFIR', var: SVFVar, succ: int, as_: 'AbstractState') -> bool: ...
    """Not a real SVF C++ AbstractState/AbstractInterpretation API: this is a pysvf-only
    static helper that reimplements AbstractInterpretation::isSwitchBranchFeasible's logic
    in the Python bindings, since that method is private in the C++ library."""
    ...
    """Check if an address is in the address-to-addresses table"""
    def initObjVar(self, objVar: ObjVar) -> None: ...
    """Initialize an object variable in the abstract state"""
    def addToFreedAddrs(self, addr: int) -> None: ...
    """Add an address to the freed addresses set"""
    def isFreedMem(self, addr: int) -> bool: ...
    """Check if an address is freed memory"""
    def hash(self) -> int: ...
    """Get the hash of this abstract state"""

    def clear(self) -> None: ...
    def clone(self) -> 'AbstractState': ...
    """Returns an instance of type(self) (subclasses are preserved), not necessarily
    a plain AbstractState."""
...
class AbstractInterpretation:
    """Owns the per-ICFGNode AbstractState trace and exposes the
    sparsity-aware GEP / load / store / size helpers that used to live on
    ...
    def getGepElementIndex(self, gep: 'GepStmt') -> IntervalValue: ...
    def getGepByteOffset(self, gep: 'GepStmt') -> IntervalValue: ...
    def getGepObjAddrs(self, pointer: 'ValVar', offset: IntervalValue) -> AddressValue: ...

    def loadValue(self, pointer: 'ValVar', node: 'ICFGNode') -> AbstractValue: ...
    def storeValue(self, pointer: 'ValVar', val: AbstractValue, node: 'ICFGNode') -> None: ...

    def getPointeeElement(self, var: 'ObjVar', node: 'ICFGNode') -> 'SVFType': ...
    def getAllocaInstByteSize(self, addr: 'AddrStmt') -> int: ...

    def getTrace(self) -> dict: ...
    def __getitem__(self, node: 'ICFGNode') -> AbstractState: ...
    def __setitem__(self, node: 'ICFGNode', state: AbstractState) -> None: ...
    def __contains__(self, node: 'ICFGNode') -> bool: ...

    def analyse(self) -> None: ...
    def analyzeFromAllProgEntries(self) -> None: ...
    def runOnModule(self) -> None: ...


class Options:
    """A small subset of SVF Options exposed for downstream Python code."""

    @staticmethod
    def max_field_limit() -> int: ...
...
class AndersenBase:
    ...
    def mayAlias(self, id1: int, id2: int) -> bool: ...
